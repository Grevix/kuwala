...
def audit_sp500_dataset():
    ...
    if close_col:
        ...
        if symbol_col and date_col:
            ...
            if extreme_jumps > 0:
                anomalies.append(
                    f"Found {extreme_jumps} extreme single-day returns (>80%) - potential unadjusted splits"
                )
    ...
    print(
        f"  [AUDIT] S&P 500: {row_count:,} rows, {asset_count} assets, range {min_date}..{max_date}, Missing: {null_pct:.2f}%"
    )
...
def audit_nasdaq_datasets():
    ...
    if f2.exists():
        ...
        print("  [AUDIT] Nasdaq-100 Intraday 1-Minute (novandra): 1-min OHLCV bars for index futures NAS100")
        manifest_entries.append(
            {
