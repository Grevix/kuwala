...
def run_real_market_validation():
    ...
    for sym in tickers:
        ...
        print(
            f"[{sym}] Butterfly Arbitrage: {'PASSED' if diag.butterfly_passed else 'FAILED'} | Calendar: {'PASSED' if diag.calendar_passed else 'FAILED'}"
        )
        ...
        print(
            f"[{sym}] Dupire Local Vol Shape: {loc_vol.shape} | Strict Positivity: {'PASSED' if not has_negative_var else 'FAILED'}"
        )
        ...
        sim_signal = pd.Series(
            np.where(hist_df["close"].pct_change().shift(-1) > 0, 1.0, -1.0), index=hist_df.index
        ).dropna()
        ...
        manifest_entries.append(
            {
                ...
                "schema_hash": hashlib.sha256(str(cleaned_chain.to_dataframe().columns.tolist()).encode()).hexdigest()[
                    :16
                ],
    ...
    print(
        f"Total Contracts: {total_real_contracts:,} | Valid: {total_valid_contracts:,} | Rejected: {total_rejected:,}"
    )
    print("=" * 75)
